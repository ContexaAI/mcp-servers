...
export class ToolsManager {
  ...
  async initialize(): Promise<void> {
    // Check if we have a valid OpenAPI spec to load
    if (
      !this.config.openApiSpec ||
      this.config.openApiSpec === "none" ||
      this.config.specInputMethod === "none"
    ) {
      console.warn("⚠️  No OpenAPI spec provided. Server will start with limited functionality.")
      this.loadedSpec = undefined

      // Add only dynamic tools when no spec is available
      const dynamicTools = this.createDynamicTools()
      for (const [toolId, tool] of dynamicTools) {
    ...
    try {
      ...
      console.warn("⚠️  Failed to load OpenAPI spec. Server will start with limited functionality.")
      console.warn(`Error: ${error instanceof Error ? error.message : String(error)}`)

      // Add only dynamic tools when spec loading fails
      this.loadedSpec = undefined
      const dynamicTools = this.createDynamicTools()
      for (const [toolId, tool] of dynamicTools) {
        this.tools.set(toolId, tool)
