 ...
 */
export async function contexaStart(server: Server | McpServer | { getServer(): Server }) {
  const app = express();
  const transports: Record<string, StreamableHTTPServerTransport> = {};

  // Get the actual MCP server instance
  const mcpServer = 'getServer' in server ? server.getServer() : server;
