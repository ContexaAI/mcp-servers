...
import { getQueryFields, graphqlRequestSchema, } from '../src/content-api/graphql.js';
...
export const mockManagementApi = [
    http.get(TRACE_URL, () => {
        return HttpResponse.text(`fl=123abc\nvisit_scheme=https\nloc=${COUNTRY_CODE}\ntls=TLSv1.3\nhttp=http/2`);
    }),
    ...
    http.all(`${API_URL}/*`, ({ request }) => {
        ...
        expect(userAgent).toBe(`${MCP_SERVER_NAME}/${MCP_SERVER_VERSION} (${MCP_CLIENT_NAME}/${MCP_CLIENT_VERSION})`);
    }),
    http.get(`${API_URL}/v1/projects`, () => {
        return HttpResponse.json(Array.from(mockProjects.values()).map((project) => project.details));
    }),
    ...
    http.post(`${API_URL}/v1/projects/:projectId/pause`, ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
    ...
    http.post(`${API_URL}/v1/projects/:projectId/restore`, ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ error: 'Project not found' }, { status: 404 });
        }
    ...
    http.get(`${API_URL}/v1/organizations`, () => {
        return HttpResponse.json(Array.from(mockOrgs.values()).map(({ id, name }) => ({ id, name })));
    }),
    http.get(`${API_URL}/v1/organizations/:id`, ({ params }) => {
        const organization = Array.from(mockOrgs.values()).find((org) => org.id === params.id);
    ...
    http.post(`${API_URL}/v1/projects/:projectId/database/query`, async ({ params, request }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
      ...
      `;
        ...
        if (!lastStatementResults) {
            return HttpResponse.json({ message: 'Failed to execute query' }, { status: 500 });
        }
        return HttpResponse.json(lastStatementResults.rows);
    }),
    http.get(`${API_URL}/v1/projects/:projectId/functions`, ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
        return HttpResponse.json(Array.from(project.edge_functions.values()).map((edgeFunction) => edgeFunction.details));
    }),
    http.get(`${API_URL}/v1/projects/:projectId/functions/:functionSlug`, ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
        ...
        if (!edgeFunction) {
            return HttpResponse.json({ message: 'Edge Function not found' }, { status: 404 });
        }
        return HttpResponse.json(edgeFunction.details);
    }),
    http.get(`${API_URL}/v1/projects/:projectId/functions/:functionSlug/body`, ({ params, request }) => {
        if (request.headers.get('Accept') !== 'multipart/form-data') {
            return HttpResponse.json({
                message: 'Invalid Accept header. Must be multipart/form-data for testing',
            }, { status: 406 });
        }
        const project = mockProjects.get(params.projectId);
        if (!project) {
    ...
    http.post(`${API_URL}/v1/projects/:projectId/functions/deploy`, async ({ params, request }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
        ...
        const metadataString = metadataFormValue instanceof File
    ...
    http.get(`${API_URL}/v1/projects/:projectId/database/migrations`, async ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.post(`${API_URL}/v1/projects/:projectId/database/migrations`, async ({ params, request }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.get(`${API_URL}/v1/projects/:projectId/analytics/endpoints/logs.all`, async ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.get(`${API_URL}/v1/projects/:projectId/advisors/security`, async ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.get(`${API_URL}/v1/projects/:projectId/advisors/performance`, async ({ params }) => {
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.post(`${API_URL}/v1/projects/:projectId/branches`, async ({ params, request }) => {
        const { branch_name } = await request.json();
        const project = mockProjects.get(params.projectId);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
        const projectBranches = Array.from(mockBranches.values()).filter((branch) => branch.parent_project_ref === project.id);
    ...
    http.get(`${API_URL}/v1/projects/:projectId/branches`, async ({ params }) => {
        const projectBranches = Array.from(mockBranches.values()).filter((branch) => branch.parent_project_ref === params.projectId);
        if (projectBranches.length === 0) {
            return HttpResponse.json({ message: 'Preview branching is not enabled for this project.' }, { status: 422 });
        }
        return HttpResponse.json(projectBranches.map((branch) => branch.details));
    }),
    http.delete(`${API_URL}/v1/branches/:branchId`, async ({ params }) => {
        const branch = mockBranches.get(params.branchId);
        if (!branch) {
            return HttpResponse.json({ message: 'Branch not found' }, { status: 404 });
        }
        if (branch.is_default) {
            return HttpResponse.json({ message: 'Cannot delete the default branch.' }, { status: 422 });
        }
    ...
    http.post(`${API_URL}/v1/branches/:branchId/merge`, async ({ params }) => {
        const branch = mockBranches.get(params.branchId);
        if (!branch) {
            return HttpResponse.json({ message: 'Branch not found' }, { status: 404 });
        }
        ...
        if (!parentProject) {
            return HttpResponse.json({ message: 'Parent project not found' }, { status: 404 });
        }
        const project = mockProjects.get(branch.project_ref);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        ...
        try {
            ...
            return HttpResponse.json({ message: 'Failed to apply migrations' }, { status: 500 });
        }
        return HttpResponse.json({ message: 'ok' });
    }),
    http.post(`${API_URL}/v1/branches/:branchId/reset`, async ({ params, request }) => {
        const branch = mockBranches.get(params.branchId);
        if (!branch) {
            return HttpResponse.json({ message: 'Branch not found' }, { status: 404 });
        }
        ...
        if (body.migration_version) {
            ...
            project.migrations = project.migrations.filter((m) => m.version <= target);
        }
    ...
    http.post(`${API_URL}/v1/branches/:branchId/push`, async ({ params }) => {
        const branch = mockBranches.get(params.branchId);
        if (!branch) {
            return HttpResponse.json({ message: 'Branch not found' }, { status: 404 });
        }
    ...
    http.get(`${API_URL}/v1/projects/:ref/storage/buckets`, ({ params }) => {
    ...
    http.get(`${API_URL}/v1/projects/:ref/config/storage`, ({ params }) => {
        const project = mockProjects.get(params.ref);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
    ...
    http.patch(`${API_URL}/v1/projects/:ref/config/storage`, async ({ params, request }) => {
        const project = mockProjects.get(params.ref);
        if (!project) {
            return HttpResponse.json({ message: 'Project not found' }, { status: 404 });
        }
...
export class MockEdgeFunction {
    async setFiles(files) {
        ...
        for (const file of files) {
            this.files.push(new File([file], `${join(this.pathPrefix, file.name)}`, {
                type: file.type,
            }));
        }
    }
    ...
    constructor(projectId, { name, entrypoint_path, import_map_path }) {
        this.files = [];
    ...
    update({ name, entrypoint_path, import_map_path }) {
        this.name = name;
        this.version += 1;
        this.entrypoint_path = `file://${join(this.pathPrefix, entrypoint_path)}`;
        this.import_map_path = import_map_path
...
export class MockProject {
    get db() {
        if (!__classPrivateFieldGet(this, _MockProject_db, "f")) {
            __classPrivateFieldSet(this, _MockProject_db, new PGlite(), "f");
            __classPrivateFieldGet(this, _MockProject_db, "f").waitReady.then(() => {
                __classPrivateFieldGet(this, _MockProject_db, "f").exec(`
          CREATE ROLE supabase_read_only_role;
          GRANT pg_read_all_data TO supabase_read_only_role;
        `);
            });
        }
        return __classPrivateFieldGet(this, _MockProject_db, "f");
    }
    get details() {
        return {
            id: this.id,
    ...
    constructor({ name, region, organization_id }) {
        this.migrations = [];
        this.edge_functions = new Map();
        this.storage_buckets = new Map();
        _MockProject_db.set(this, void 0);
        this.id = nanoid();
        this.name = name;
        this.region = region;
    ...
    async resetDb() {
        if (__classPrivateFieldGet(this, _MockProject_db, "f")) {
            await __classPrivateFieldGet(this, _MockProject_db, "f").close();
        }
        __classPrivateFieldSet(this, _MockProject_db, undefined, "f");
        return this.db;
    }
    async deployEdgeFunction(options, files = []) {
    ...
    createStorageBucket(name, isPublic = false) {
        const id = nanoid();
        const bucket = {
            id,
            name,
...
export class MockBranch {
    ...
    constructor({ name, project_ref, parent_project_ref, is_default, }) {
        this.id = nanoid();
        this.name = name;
