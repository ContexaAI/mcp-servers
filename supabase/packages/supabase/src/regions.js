...
export const AWS_REGION_CODES = Object.values(AWS_REGIONS).map((region) => region.code);
export function getClosestAwsRegion(location) {
    const distances = Object.entries(AWS_REGIONS).map(([name, region]) => {
        return [name, getDistance(location, region.location)];
    });
    const closestRegion = distances.reduce((min, current) => min === undefined ? current : current[1] < min[1] ? current : min, undefined);
    if (!closestRegion) {
...
export function getDistance(a, b) {
    ...
    const a1 = Math.sin(lat / 2) * Math.sin(lat / 2) +
