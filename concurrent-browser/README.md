...
concurrent-browser-mcp uses a structured JSON configuration for environment variables. The configuration is stored in `env` and `env.example` files in the project root.

### Environment Variable Structure

The environment variables are defined in JSON format with the following structure:
...
|----------|-------------|----------|---------|
| `HTTP_PROXY` / `http_proxy` | HTTP proxy server address | No | null |
| `HTTPS_PROXY` / `https_proxy` | HTTPS proxy server address | No | null |
| `ALL_PROXY` / `all_proxy` | Universal proxy server address | No | null |

### Configuration Files

- **`env`**: Main environment configuration file (should not be committed to version control)
- **`env.example`**: Example configuration file (safe to commit to version control)

### Usage

1. **Copy the example file**: `cp env.example env`
2. **Edit the env file**: Set your environment variable values
3. **Set actual environment variables**: The system will prioritize actual environment variables over the JSON configuration

### Priority Order

1. Actual environment variables (e.g., `process.env.HTTP_PROXY`)
2. Values defined in the `env` file
3. Values defined in the `env.example` file (fallback)
...
The environment configuration test verifies that the new JSON-based environment variable system works correctly:
...
This test will:
1. **Load configuration**: Test loading from `env` and `env.example` files
2. **Check variables**: Verify environment variable retrieval
3. **Test proxy config**: Validate proxy configuration detection
4. **Priority testing**: Test the priority order of environment variables
