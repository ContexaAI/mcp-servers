...
async function initializeFirebase(): Promise<admin.app.App | null> {
  try {
    ...
    if (!serviceAccountPath) {
      logger.warn('SERVICE_ACCOUNT_KEY_PATH not set, Firebase features may not work properly');
      return null;
    }
...
class FirebaseMcpServer {
            ...
            content: [
              {
                type: 'text',
                text: JSON.stringify({
                  error: 'Firebase not initialized - check SERVICE_ACCOUNT_KEY_PATH configuration',
                }),
              },
            ],
          };
  ...
  /**
   * Starts the MCP server using the Contexa transport.
   * This method initializes the Contexa transport for the server.
   */
  async run(): Promise<void> {
    // Skip Firebase initialization wait - just log warning and continue
    if (!app) {
      logger.warn('Firebase not initialized - some features may not work properly');
    }

    logger.info(
      `Starting Firebase MCP server v${config.version} with Contexa transport`
    );
