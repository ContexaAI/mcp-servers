...
class PersistentContextFactory implements BrowserContextFactory {
  ...
  async createContext(): Promise<{ browserContext: playwright.BrowserContext, close: () => Promise<void> }> {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        if (error.message.includes('Executable doesn\'t exist')) {
          console.warn(`Warning: Browser specified in your config is not installed. Either install it or change the config. Continuing with default browser.`);
          // Try to use a default browser
          const defaultBrowser = playwright.chromium;
          const browserContext = await defaultBrowser.launchPersistentContext(userDataDir, {
            ...this.browserConfig.launchOptions,
            ...this.browserConfig.contextOptions,
            handleSIGINT: false,
            handleSIGTERM: false,
    ...
    console.warn(`Warning: Browser is already in use for ${userDataDir}, use --isolated to run multiple instances of the same browser. Continuing with limited functionality.`);
    // Return a dummy context that will handle the case when browser is already in use
    return {
  ...
  private async _createUserDataDir() {
    ...
    if (process.platform === 'linux')
      cacheDirectory = getEnv('XDG_CACHE_HOME') || path.join(os.homedir(), '.cache');
    else if (process.platform === 'darwin')
      cacheDirectory = path.join(os.homedir(), 'Library', 'Caches');
    else if (process.platform === 'win32')
      cacheDirectory = getEnv('LOCALAPPDATA') || path.join(os.homedir(), 'AppData', 'Local');
      else {
    console.warn('Warning: Unsupported platform: ' + process.platform + ', using default cache directory.');
