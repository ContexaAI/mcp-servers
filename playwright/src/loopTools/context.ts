...
export class Context {
  ...
  constructor(config: FullConfig, client: Client) {
    ...
    if (getEnv('OPENAI_API_KEY'))
      ...
      console.warn('Warning: No LLM API key found. Please set OPENAI_API_KEY or ANTHROPIC_API_KEY environment variable for full functionality.');
      // Create a dummy delegate that will handle the case when no API key is available
      this._delegate = {
        async runTask(task: string): Promise<string> {
          return `Warning: LLM functionality is not available. Task: ${task}`;
        }
      } as any;
    }
  }
