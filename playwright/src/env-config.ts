...
export function loadEnvConfig(): EnvConfig {
  try {
    ...
    const envVariables: EnvVariable[] = JSON.parse(envConfigContent);
 ...
 */
export function getBooleanEnv(key: string): boolean | undefined {
  const value = getEnv(key);
 ...
 */
export function getCommaListEnv(key: string): string[] | undefined {
  const value = getEnv(key);
 ...
 */
export function getSemicolonListEnv(key: string): string[] | undefined {
  const value = getEnv(key);
  if (!value) return undefined;
  return value.split(';').map(v => v.trim());
} 