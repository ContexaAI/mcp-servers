...
export async function main() {
  ...
  if (!apiKeyName || !privateKey) {
    console.warn(
      'Warning: COINBASE_API_KEY_NAME and COINBASE_API_PRIVATE_KEY environment variables are not set. Some features may be limited.',
    );
  }
  ...
  if (apiKeyName && privateKey) {
    ...
    toolHandler = mcpTools.toolHandler;
  } else {
    console.warn('AgentKit features disabled due to missing API credentials');
  }
      ...
      if (!toolHandler) {
        return {
          content: [
            {
              type: 'text',
              text: 'ERROR: AgentKit tools are not available. Please set COINBASE_API_KEY_NAME and COINBASE_API_PRIVATE_KEY environment variables.',
            },
          ],
        };
      }
  ...
  console.error('Starting Contexa transport...');
