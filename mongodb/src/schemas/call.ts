...
export async function handleCallToolRequest({
  ...
  if (!client || !db) {
    ...
    return { content: [{ type: "text", text: "MongoDB connection is not available" }] };
  }
  ...
  if (COLLECTION_OPERATIONS.includes(operation)) {
    ...
    if (!db) {
      console.warn("MongoDB database connection is not available");
      return { content: [{ type: "text", text: "MongoDB database connection is not available" }] };
    }
  ...
  switch (operation) {
    ...
    case "serverInfo":
      if (!db) { console.warn("MongoDB database connection is not available"); return { content: [{ type: "text", text: "MongoDB database connection is not available" }] }; }
