...
if (!process.env.API_TOKEN) {
    console.warn('⚠️  WARNING: API_TOKEN environment variable is not set. Browser tools may not work properly.');
}
...
const calculate_cdp_endpoint = async()=>{
    if (!process.env.API_TOKEN) {
        throw new Error('API_TOKEN is required for browser functionality. Please set the API_TOKEN environment variable.');
    }
...
let scraping_browser_activation_instructions = {
    ...
    parameters: z.object({}),
    execute: async()=>{
        return 'You need to run this MCP server with the API_TOKEN '
        +'environment variable before the browser tools will become '
        +'available';
    },
};
