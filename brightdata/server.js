...
if (!api_token) {
    console.warn('⚠️  WARNING: API_TOKEN environment variable is not set. Some features may not work properly.');
}
if (!process.env.WEB_UNLOCKER_ZONE) {
    console.warn('⚠️  WARNING: WEB_UNLOCKER_ZONE environment variable is not set. Using default: mcp_unlocker');
}
if (!process.env.BROWSER_ZONE) {
    console.warn('⚠️  WARNING: BROWSER_ZONE environment variable is not set. Using default: mcp_browser');
}
if (!process.env.RATE_LIMIT) {
    console.warn('⚠️  WARNING: RATE_LIMIT environment variable is not set. Rate limiting will be disabled.');
}
...
function parse_rate_limit(rate_limit_str) {
    ...
    if (!match) {
        console.warn('⚠️  WARNING: Invalid RATE_LIMIT format. Use: 100/1h or 50/30m. Rate limiting will be disabled.');
        return null;
    }
...
const api_headers = ()=>{
    if (!api_token) {
        console.warn('⚠️  WARNING: API_TOKEN not available. API calls will fail.');
        return {
...
async function ensure_required_zones(){
    if (!api_token) {
        console.warn('⚠️  WARNING: Skipping zone setup - API_TOKEN not available');
        return;
    }
