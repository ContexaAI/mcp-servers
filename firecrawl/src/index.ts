...
function safeLog(
  ...
  try {
    // Check if server is connected before trying to send logging message
    if (server && typeof server.sendLoggingMessage === 'function') {
      server.sendLoggingMessage({ level, data });
    } else {
      // Fall back to console logging if MCP connection is not established
      console.log(`[${level.toUpperCase()}]`, data);
    }
  } catch (error) {
    // Fall back to console logging if MCP connection fails
    console.log(`[${level.toUpperCase()}]`, data);
  }
}
...
// Server startup with Contexa transport
async function startServer() {
  try {
    console.log('Initializing Firecrawl MCP Server with Contexa transport...');
