...
const configuration: Config = {
  clickupApiKey: envArgs.clickupApiKey || process.env.CLICKUP_API_KEY || 'demo-key',
  clickupTeamId: envArgs.clickupTeamId || process.env.CLICKUP_TEAM_ID || 'demo-team',
...
const missingEnvVars = requiredVars
  .filter(key => {
    const value = configuration[key as keyof Config];
    return !value || value === 'demo-key' || value === 'demo-team';
  })
  .map(key => key);

if (missingEnvVars.length > 0) {
  console.warn(`Warning: Missing required environment variables: ${missingEnvVars.join(', ')}. Using demo values.`);
}
